...
# Cifar 5M MIXED training experiment script 
...
Mixed training consists in training the model with DRM first to reach the teacher performance, and using the remaining data
by training with ERM.  
...
def setup_optimizerNscheduler(args, model, stud=False, second_time=False):
        lr = args.lr
        if stud: 
        ...
        if not args.optim_cosineanneal: 
                ...
                scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=epochs-args.optim_warmup, eta_min=1e-4)
...
def save_checkpoint(state, is_best, filename='checkpoint.pth.tar'):
    path = base_path() + "/chkpts" + "/" + "cifar10mix" + "/" + "mnet/"
...
def parse_args(buffer=False):
    ...
    parser.add_argument('--distil_proportion', type=float, default=0.4, help='Portion of the dataset to be used in DRM training. ')
    parser.add_argument('--reset_optim', default=False, action='store_true', help='Reset the optimizeer when starting the second training phase.')
    parser.add_argument('--teacher_off', default=False, action='store_true', help='ERM only training (but doubling the number of epochs)')
...
print(f"Randomly splitting Cifar10 in two")
teacher.eval() # set the main model to evaluation
all_indices = set(range(len(C10_train)))
random_indices = np.random.choice(list(all_indices), 
                size=len(C10_train), replace=False)

distil_subset = int(args.distil_proportion * len(C10_train))

d_train_subset = Subset(C10_train, random_indices[:distil_subset])
e_train_subset = Subset(C10_train, random_indices[distil_subset:])
...
if not args.teacher_off:
        print("Starting DRM training phase...")
else: print("Starting first ERM phase")

loader =  DataLoader(d_train_subset, 
...
for e in range(args.n_epochs_stud):
        ...
        for i, data in enumerate(loader):
                ...
                if args.teacher_off:
                       if args.MSE: 
                                loss = F.mse_loss(outputs, F.one_hot(labels, num_classes=10).to(torch.float) * LOGITS_MAGNITUDE_TEACHER)  # Bobby's correction
                       else:
                                loss = F.cross_entropy(outputs, labels)
                else: 
                        loss = vanilla_distillation(outputs, logits)
                loss.backward()
                optimizer.step()

                assert not math.isnan(loss)
                progress_bar.prog(i, len(loader), e, 'Student-D', loss.item())
                avg_loss += loss
...
if args.reset_optim:
        optimizer, scheduler = setup_optimizerNscheduler(args, student, stud=True, second_time=True)


print("Starting ERM training phase...")

loader =  DataLoader(e_train_subset, 
                batch_size=args.batch_size, 
                shuffle=True, 
                num_workers=2,  
                pin_memory=False)

for e in range(args.n_epochs_stud, args.n_epochs_stud*2):
        if args.debug_mode and e > args.n_epochs_stud+3: # only 3 batches in debug mode
                break
        avg_loss = 0.0
        correct, total, agreement, function_distance = 0.0, 0.0, 0.0, 0.0
        for i, data in enumerate(loader):
                ...
                assert not math.isnan(loss)
                progress_bar.prog(i, len(loader), e, 'Student-E', loss.item())
                avg_loss += loss
...
if args.checkpoints: 
        ...
        'optimizer' : optimizer.state_dict(),
        'scheduler' : scheduler.state_dict()
        }, False, filename=f'mnet-student-mixed-{args.seed}.ckpt')
...
path = base_path() + "results" + "/" + "cifar10mix" + "/" + "mnet" 
if not os.path.exists(path): os.makedirs(path)
