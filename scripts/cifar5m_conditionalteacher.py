...
if not args.nowand:
        ...
        if args.wandb_name is None: 
                name = str.join("-",["cifar5m", "convnet", args.conf_timestamp])
...
if not args.pretrained:
        ...
        for epoch in range(start_epoch, args.n_epochs):
                ...
                for i, data in enumerate(train_loader):
                        ...
                        if args.MSE: 
                                loss = F.mse_loss(outputs, F.one_hot(labels, num_classes=10).to(torch.float)) 
                        else:
...
experiment_log['network'] = 'convnet'
...
path = base_path() + "results" + "/" + "cifar5m" + "/" + "convnet" 
if not os.path.exists(path): os.makedirs(path)
